...
import ThemeSegmented, { ThemeMode } from '../components/ThemeSegmented'
...
export type AlertReviewDecision = 'false-alert' | 'valid-alert'
type ColorblindMode = 'off' | 'deuteranopia' | 'protanopia' | 'tritanopia'
...
import parkingLotPreview from '../assets/parkinglot.jpg'
...
function getRouteBucket(pathname: string): '/' | '/alerts' | '/cameras' | '/help' | '/other' {
  if (pathname === '/') return '/'
  if (pathname.startsWith('/alerts')) return '/alerts'
  if (pathname.startsWith('/cameras')) return '/cameras'
  if (pathname.startsWith('/camera/')) return '/cameras'
...
export default function App() {
  ...
  const routeOrder = useMemo<Record<string, number>>(() => ({
    '/': 0,
    '/alerts': 1,
    '/cameras': 2,
    '/help': 3,
  ...
  const [routeAnimDir, setRouteAnimDir] = useState<'forward' | 'backward'>('forward')
  const [routeAnimProfile, setRouteAnimProfile] = useState<'default' | 'heavy'>('default')
  const [isRouteAnimating, setIsRouteAnimating] = useState(false)
  const routeAnimTimeoutRef = useRef<number | null>(null)
  ...
  const homePanelTimerRef = useRef<number | null>(null)
  const searchPlaceholder = useMemo(() => {
    if (location.pathname.startsWith('/alerts')) return 'Search alerts…'
    if (location.pathname.startsWith('/help')) return 'Search help…'
    if (location.pathname.startsWith('/cameras')) return 'Search cameras…'
  ...
  useEffect(() => {
    ...
    setRouteAnimProfile(
      (prevBucket === '/' && nextBucket === '/cameras') || (prevBucket === '/cameras' && nextBucket === '/')
        ? 'heavy'
        : 'default',
    )
  ...
  const [groups, setGroups] = useState<CameraGroup[]>(() => {
    try {
      ...
      const parsed = JSON.parse(raw) as CameraGroup[]
      if (!Array.isArray(parsed) || parsed.length === 0) return DEFAULT_GROUPS
      return parsed
        .filter((g): g is CameraGroup => !!g && typeof g.id === 'string' && typeof g.name === 'string')
  ...
  const [cameras, setCameras] = useState<Camera[]>(() => {
    const pinned = new Set<string>(JSON.parse(localStorage.getItem(PINNED_KEY) || '[]'))
    return [
      { id: 'parking-lot', name: 'Parking Lot', location: 'Exterior', preview: parkingLotPreview, pinned: pinned.has('parking-lot'), online: true, groupIds: ['parking-lot'] },
  ...
  const saveCameraDetails = (name: string, location: string, editingId: string | null, groupIds: string[] = []) => {
  ...
  const togglePin = (id: string) => {
    setCameras(prev => {
      ...
      if (updated.pinned) return [updated, ...rest]
  ...
  const addGroup = (name: string): string | null => {
  ...
  const assignCameraGroups = (cameraId: string, groupIds: string[]) => {
  ...
  const [alertStatusOverrides, setAlertStatusOverrides] = useState<Record<string, string>>({})
  const [alertReviewDecisions, setAlertReviewDecisions] = useState<Record<string, AlertReviewDecision>>({})
  const alertEvents = useMemo(
    () => buildAlertEvents(cameras, alertStatusOverrides),
    [cameras, alertStatusOverrides],
  )
  ...
  const [colorblindMode, setColorblindMode] = useState<ColorblindMode>(() => {
    ...
    if (saved === '1') return 'deuteranopia'
    return saved === 'deuteranopia' || saved === 'protanopia' || saved === 'tritanopia' ? saved : 'off'
  })
  const [draftColorblindMode, setDraftColorblindMode] = useState<ColorblindMode>(colorblindMode)
  const effectiveTheme = useMemo(() => settingsOpen ? draftTheme : theme, [settingsOpen, draftTheme, theme])
  const [homeModalAlertId, setHomeModalAlertId] = useState<string | null>(null)
  const [homeModalExitVariant, setHomeModalExitVariant] = useState<'default' | 'to-camera'>('default')
  ...
  useEffect(() => {
    ...
    if (colorblindMode !== 'off') document.documentElement.setAttribute('data-colorblind', colorblindMode)
  ...
  const reviewAlert = (id: string, decision: AlertReviewDecision) => {
    setAlertStatusOverrides((prev) => ({ ...prev, [id]: 'Reviewed' }))
    setAlertReviewDecisions((prev) => ({ ...prev, [id]: decision }))
  }
  const homeModalAlert = useMemo(
    () => alertEvents.find((a) => a.id === homeModalAlertId) ?? null,
    [alertEvents, homeModalAlertId],
  )
  const closeHomeAlertModal = () => {
    setHomeModalExitVariant('default')
    setHomeModalAlertId(null)
  }
  const viewCameraFromHomeAlert = (alert: (typeof alertEvents)[number]) => {
  ...
  const openClipFromHomeAlert = (alert: (typeof alertEvents)[number]) => {
    setHomeModalExitVariant('to-camera')
    setHomeModalAlertId(null)
  ...
  return (
    ...
    >
      ...
      <header className="topBar">
        <div className="topBarLeft">
          <div className="topBarLogo">
            <div className="topBarLogoMark"><IconLogo /></div>
            <span className="topBarWordmark" aria-label="Sense">
              {['S', 'e', 'n', 's', 'e'].map((ch, i) => (
                <span key={`${ch}-${i}`} className="topBarWordmarkChar" style={{ ['--char-i' as string]: i } as CSSProperties}>
                  {ch}
                </span>
              ))}
            </span>
        ...
        <div className={`contentArea${isHomeRoute ? ' homeContentArea' : ''}`}>
          <div className="routeSurface">
            <Routes>
              <Route
                path="/"
                element={<MainPage {...cameraPageProps} />}
              />
              <Route
                path="/alerts"
                element={
                  <AlertsPage
                    cameras={cameras}
                    alerts={alertEvents}
                    reviewDecisions={alertReviewDecisions}
              ...
              <Route path="/help" element={<HelpPage searchQuery={searchQuery} />} />
      ...
      <AlertDetailsModal
        alert={homeModalAlert}
        onClose={closeHomeAlertModal}
        onViewCamera={viewCameraFromHomeAlert}
        onOpenClip={openClipFromHomeAlert}
        onReviewAlert={reviewAlert}
        reviewDecision={homeModalAlert ? alertReviewDecisions[homeModalAlert.id] : undefined}
        exitVariant={homeModalExitVariant}
      />
