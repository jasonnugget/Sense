...
type ThreatLevel = AlertEvent['level']
...
function useMockEvents(): AlertEvent[] {
  return useMemo(() => [
...
const THREAT_DOT_COLORS: Record<ThreatLevel, string> = {
  high:   'var(--danger)',
  medium: 'var(--warning)',
  low:    'var(--accent)',
}

export default function ActivityPanel({ events, onEventClick }: Props) {
      ...
      <div className="eventList">
        {display.map(event => (
          <button
            key={event.id}
            type="button"
            className={`eventRow${onEventClick ? ' eventRowInteractive' : ''}`}
            onClick={() => onEventClick?.(event)}
          >
            <div
              className="eventPip"
              style={{ background: THREAT_DOT_COLORS[event.level] }}
            />
